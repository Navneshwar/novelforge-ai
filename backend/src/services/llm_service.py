...
class LLMService:
    ...
    async def generate_with_memory(self, prompt: str, memory_context: List[Dict], **kwargs) -> str:
...
"""
            ...
            # Parse response
            characters = [line.strip() for line in response.response.split("\n") if line.strip()]
            return characters
    ...
    async def check_consistency(self, text: str, memory_context: List[Dict]) -> Dict:
        ...
        try:
            ...
            ]) if memory_context else "No memory context available."
